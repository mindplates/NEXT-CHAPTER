...
- ecc 플러그인 훅은 TypeScript 전제로 짜여 있다(edit 시 Prettier/tsc). **백엔드 쪽은 스택에
  맞게 커스터마이징이 필요하다** — 포크의 `hooks/hooks.json`을 수정한다
- 데이터스토어, 벡터 검색 수단, 비동기 잡 러너, LLM 프로바이더는 미정
...
방향은 위에서 정해졌고, 아래는 그 방향 안에서 남은 선택지다.

- **초기 카탈로그 시드** — 도메인·분야를 어디까지 미리 채워둘 것인가. 비어 있으면 초기
  사용자 전원이 신규 뼈대를 만들어 통합이 작동하지 않는다
- **집단 루프 임계치** — "몇 명이 같은 지점에서 물어야" 수정을 트리거하는가. 초기 사용자가
  적을 때도 루프가 한 바퀴 돌 수 있는 수치여야 한다
- **개인 레이어의 변형 방식** — 레이어가 공용 뼈대를 구체적으로 어떻게 변형해 화면을
  만드는가 (난이도 조절, 보충 설명 삽입, 챕터 순서 재배열 등)
- **인프라** — 데이터스토어, 벡터 검색, 비동기 잡 러너, LLM 프로바이더
- **생성 비용 상한** — 뼈대 하나당 허용 토큰·금액. 검증 패스를 전 챕터에 돌릴지 샘플링할지가
  여기에 딸린다
